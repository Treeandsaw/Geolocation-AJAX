...
mysql_select_db('geolocation') or die(mysql_error());
...
*/
$query = "SELECT *,((ACOS(SIN($latitude * PI() / 180) * SIN(latitude * PI() / 180) + COS($latitude * PI() / 180) * COS(latitude * PI() / 180) * COS(($longitude - longitude) * PI() / 180)) * 180 / PI()) * 60 * 1.1515) AS `distance` FROM businesses HAVING distance < 100 ORDER by distance ASC";
